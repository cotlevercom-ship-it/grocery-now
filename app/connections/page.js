...
export default function ConnectionsPage() {
  ...
  const load = useCallback(async (uid) => {
    setLoading(true)
    try {
      const conns = await supabaseFetch(
        `connections?select=id,requester_id,addressee_id,responded_at&status=eq.accepted&or=(requester_id.eq.${uid},addressee_id.eq.${uid})&order=responded_at.desc`
      )
      const otherIds = (conns || []).map(c => c.requester_id === uid ? c.addressee_id : c.requester_id)
      let profilesById = {}
      ...
      setRows((conns || []).map(c => {
        ...
        return { id: c.id, profile: profilesById[otherId] || { user_id: otherId } }
      }))
  ...
  useEffect(() => {
    ...
    if (!uid) { router.replace('/login?next=/connections'); return }
    setUserId(uid)
    load(uid)
  }, [router, load])
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: sc.bg }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ maxWidth: '640px', margin: '0 auto', padding: 'clamp(20px,3vw,32px) clamp(16px,3vw,24px) 70px' }}>
          <h1 style={{ fontFamily: theme.fontDisplay, fontWeight: '700', fontSize: '24px', color: sc.text, marginBottom: '4px' }}>Connections</h1>
          <p style={{ fontSize: '13.5px', color: sc.textSoft, marginBottom: '20px' }}>
            {loading ? 'Loading…' : `${rows.length} connection${rows.length === 1 ? '' : 's'}.`}
          </p>

          {loading ? null : rows.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px 0', color: sc.textSoft, fontSize: '13.5px' }}>
              No connections yet. Visit a member&apos;s profile and tap <b>Connect</b> to send a request.
            </div>
          ) : (
              ...
              {rows.map(r => (
                <div key={r.id} style={{
                  ...
                  <div style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
                    <Link href={`/messages/${r.profile.user_id}`} style={{
                      background: theme.brass, color: '#FFFFFF', textDecoration: 'none', borderRadius: '8px',
                      padding: '8px 14px', fontSize: '12.5px', fontWeight: '700',
                    }}>Message</Link>
                    <button type="button" onClick={() => removeConnection(r.id)} disabled={busyId === r.id} style={{
      ...
      <AppBottomNav active="connections" />
