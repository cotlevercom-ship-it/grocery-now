...
// Updates the logged-in user's password via GoTrue. Requires a valid access
// token (caller should re-verify the current password first with
// verifyPassword before calling this, since this endpoint alone doesn't ask
// for it).
export async function updatePassword(newPassword, _retried = false) {
  const token = getAccessToken()
  ...
  if (res.status === 401 && !_retried && getSession()?.refresh_token) {
    const newToken = await refreshAccessToken()
    if (newToken) {
      return updatePassword(newPassword, true)
    }
  }

  const data = await res.json()
  if (!res.ok) throw new Error(data.error_description || data.msg || data.error || 'Password update failed')
  return data
}
...
// bump
// bump
// redeploy trigger 1786735614
// redeploy trigger 1786736387
// redeploy trigger 1786742194
// redeploy trigger 1787151331
